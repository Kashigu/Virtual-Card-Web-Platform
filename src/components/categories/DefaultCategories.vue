...
async function reloadCategories(pageNum) {
    try {
        ...
        const response = await categoryStore.loadDefaultCategoriesPaginate(pageNum, typeFilter.value, nameFilter.value)
...
async function deleteCategory(category) {
    try {
        await categoryStore.deleteDefaultCategory(category)
...
<template>
    <div class="flex flex-col w-full">
        ...
        <div class="text-3xl font-bold mt-8 mb-4 text-center">Default Categories</div>
        <div v-if="isLoading === false">
            <div class="flex gap-8 mb-4">
                ...
                <div class="ml-auto gap-4 flex flex-row">
                    <div @click="reloadCategories(1); router.push({ name: 'DefaultCategories', query: { page: 1 } });" class="gap-2 py-2 px-4 bg-blue-600 shadow text-white rounded-full justify-center flex items-center select-none cursor-pointer">
                        <i class="fas fa-search text-white"></i>
                        <div class="w-fit">Search</div>
                    </div>
                    <div @click="router.push({ name: 'AddDefaultCategory' })" class="gap-2 py-2 px-4 bg-blue-600 shadow text-white rounded-full  justify-center flex items-center select-none cursor-pointer">
            ...
            <div v-else v-for="(category) in categoryStore.categories" :class="category.type === 'C' ? 'bg-green-100' : 'bg-orange-100'" class="flex w-full py-8 mb-4 mx-auto px-4" :key="category.id">
                ...
                <div class="flex w-fit ml-auto gap-2">
                    <router-link :to="{ name: 'EditDefaultCategory', params: { id: category.id } }" class="py-2 px-4 bg-yellow-500 font-semibold shadow text-white rounded-full select-none ml-auto justify-center flex items-center gap-2"><i class="fas fa-edit text-white"></i>Edit</router-link>
            ...
            <Paginator :totalPages="totalPages" :currentPage="currentPage" :pageName="'DefaultCategories'" :reloadFunction="reloadCategories" />
        </div>
