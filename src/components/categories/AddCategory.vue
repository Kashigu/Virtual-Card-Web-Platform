...
async function save(c) {
    try {
        const response = await categoryStore.addCategory(c.name, c.type)
        //console.log(response)
        toast.success('Category ' + c.name + ' was created successfully!')
    } catch (error) {
        console.error(error)
        const t = c.type == 'C' ? 'Credit' : 'Debit'
        if (error.response.status === 409) toast.error(t + " category " + c.name + ' already exists!')
